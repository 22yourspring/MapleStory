...
const long long CGatekeeperScript::GATEKEEPER_MAX_HP = 2268000;
const long long CGatekeeperScript::GATEKEEPER_MAX_MP = 3000;

CGatekeeperScript::CGatekeeperScript()
	: CScript(CGatekeeperScript::GATEKEEPER_MAX_HP, CGatekeeperScript::GATEKEEPER_MAX_MP, CGatekeeperScript::GATEKEEPER_MAX_HP, CGatekeeperScript::GATEKEEPER_MAX_MP, (UINT)SKILL_TYPE::NONE, false, false, false, false, false, false, false, false, 0, 0.f, 0, 0.f, 0, 0.f, 0, 0.f, 0, 0.f, 0, 0.f, 0, 0.f, 0, 0, 0)
	, m_Left(true)
	, m_bTracing(false)
	, Patrol_On(false)
	, m_bPatrolLeft(true)
	...
	, DistanceValue(0.f)
{
	SetScriptType((UINT)SCRIPT_TYPE::GATEKEEPERSCRIPT);
}
...
bool CGatekeeperScript::LeftFromPlayer()
{
...
void CGatekeeperScript::state()
{
	...
	case MONSTER_FSM::ATTACK:
		Animator2D()->PlayAnim(L"8160000.img.Attack.frames");
		break;
	...
	case MONSTER_FSM::HIT:
		Animator2D()->PlayAnim(L"8160000.img.hit1");
		break;

	case MONSTER_FSM::DIE:
		Animator2D()->PlayAnim(L"8160000.img.die1.frames");
		break;

	case MONSTER_FSM::MOVE:
		Animator2D()->PlayAnim(L"8160000.img.move.frames");
		break;

	case MONSTER_FSM::STAND:
		Animator2D()->PlayAnim(L"8160000.img.stand.frames");
		break;
	...
	case MONSTER_FSM::REGENERATION:
		Animator2D()->PlayAnim(L"8160000.img.Regeneration.frames");
		break;
...
void CGatekeeperScript::ChangeState()
{
	if (DistanceCheck)
	{
		...
		else
		{
			if (m_MonsterState == MONSTER_FSM::ATTACK)
			{
				if (Animator2D()->GetCurAnim()->IsFinish())
				{
					...
					((CScript*)((CPlayerScript::PlayerObject)->GetScript()[0]))->SetSkillType((UINT)SKILL_TYPE::GATEKEEPER_ATTACK);

					if (GetGameObject()->Transform()->GetLocalPos().x <= CPlayerScript::PlayerPos.x)
					{
						CPlayerScript::PushRight = true;
					}
					else
					{
						CPlayerScript::PushLeft = true;
					}
					...
					if (CSceneMgr::GetInst()->GetLayer(L"EFFECT_DUMMY")->GetObjectVec()[0]->GetName() == L"EFFECT")
					{
						if (CSceneMgr::GetInst()->GetLayer(L"EFFECT_DUMMY")->GetObjectVec()[0]->GetChild().size() != 0)
						{
							...
							((CEffectScript*)EffectPtr->GetScript()[0])->EFFECT_SkillState = SKILL_TYPE::GATEKEEPER_ATTACK;
...
void CGatekeeperScript::SectionMove()
{
	m_fSleepAcc += DT;
...
void CGatekeeperScript::PlayerTracing()
{
...
void CGatekeeperScript::PatrolLeft()
{
	if (m_bPatrolLeft)
	{
		if (TerrainPos.x - (TerrainScale.x * 0.5f) >= Transform()->GetLocalPos().x - (Transform()->GetLocalScale().x * Collider()->GetOffsetScale().x) * 0.1f)
...
void CGatekeeperScript::PatrolRight()
{
	if (m_bPatrolRight)
	{
		if (TerrainPos.x + (TerrainScale.x * 0.5f) <= Transform()->GetLocalPos().x + (Transform()->GetLocalScale().x * Collider()->GetOffsetScale().x) * 0.1f)
...
void CGatekeeperScript::OnCollisionEnter(CCollider * _pOther)
{
...
void CGatekeeperScript::OnCollision(CCollider * _pOther)
{
	if (CSceneMgr::GetInst()->GetCurScene()->GetLayer(L"TERRAIN") == CSceneMgr::GetInst()->GetCurScene()->GetLayer(_pOther->GetGameObject()->GetLayerIdx()))
	{
		float TerrainPosY = _pOther->GetGameObject()->Transform()->GetLocalPos().y;
...
void CGatekeeperScript::OnCollisionExit(CCollider * _pOther)
{
