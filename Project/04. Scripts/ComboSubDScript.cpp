...
CComboSubDScript::CComboSubDScript() : 
	Theta (144.f * _DEGREE)
{
	SetScriptType((UINT)SCRIPT_TYPE::COMBOSUBDSCRIPT);
}
...
void CComboSubDScript::Skillstate()
{
	switch (m_SkillState)
	{
	...
	case COMBO_SUB::PHASE1:
		...
		Animator2D()->PlayAnim(L"COMBO_D");
		break;

	case COMBO_SUB::PHASE2:

		Animator2D()->PlayAnim(L"ADVANCED_COMBO_D");
		break;
...
void CComboSubDScript::SkillPos()
{
...
void CComboSubDScript::SkillChangeState()
{
	...
	if (4 <= COMBO_COUNT && COMBO_COUNT <= 8)
	{
...
void CComboSubDScript::ThetaUpdate()
{
	if (Theta == 360.f)
	{
		Theta = 0.f;
